...
namespace LibTwoTribes
{
    public abstract class ETX : Asset
    {
        ...
        public static ETX FromStream(Stream stream)
        {
            ...
            AssetHeader asset_header = new AssetHeader(stream);
            stream.Position = pos;

            switch (asset_header.EngineVersion)
            {
                case AssetUtil.EngineVersion.Version1804_Edge: return new ETX1804(stream);
                case AssetUtil.EngineVersion.Version0004_TT2: return new ETX0004(stream);
                default:
                    if (((long)asset_header.EngineVersion & 0xFF) == 0x03) return new ETX1803(stream);
                    throw new InvalidDataException("Unrecognised engine version " + AssetUtil.GetEngineVersionName(asset_header.EngineVersion));
            }
        }
        ...
        protected static unsafe byte[] _Serialize(Bitmap bmp)
        {
            ...
            BitmapData bitsBmp = bmp.LockBits(new Rectangle(new Point(0, 0), bmp.Size), ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
